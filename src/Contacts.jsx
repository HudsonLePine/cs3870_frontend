...
useEffect(() => {
  const fetchContacts = async () => {
    try {
      const response = await fetch("https://cs3870-backend-1-vu10.onrender.com/contacts");
      if (!response.ok) {
    ...
    <div className="container">
      ...
      <ul className="list-group">
        {contacts.map((contact, index) => (
          <li key={contact.id || index} className="list-group-item d-flex align-items-center">
            {contact.image_url && (
              <img
